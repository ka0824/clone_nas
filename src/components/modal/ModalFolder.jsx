...
import { removeModal, changeZindex } from "../../store/slice/ModalSlice";

const ModalFolder = ({ id }) => {
  const posX = useRef(0);
  const posY = useRef(0);
  ...
  return (
    <Wrapper info={"userInfo"} id={id} zIndex={zIndex} onClick={handleClick}>
      <TopBar
...
const Wrapper = styled.div`
  ...
  top: ${({ id }) => {
    return `calc(45% + ${id * 10}px)`;
  }};

  left: ${({ id }) => {
    return `calc(50% + ${id * 10}px)`;
  }};
